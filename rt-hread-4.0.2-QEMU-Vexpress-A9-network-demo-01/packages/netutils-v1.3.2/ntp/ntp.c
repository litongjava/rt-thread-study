...
#define SW_VER_NUM             0x00000000
    ...
    const char month[][NTP_INTERNET_MONTH_LEN] = { "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct",
        "Nov", "Dec" };
...
#define netdev_default netif_default
  ...
  while (rt_tick_get() - start < NTP_GET_TIMEOUT * RT_TICK_PER_SECOND) {
    for (i = 0; i < server_num; i++) {
      ...
      n = recvfrom(sockfd, (char *) &packet, sizeof(ntp_packet), MSG_DONTWAIT, (struct sockaddr *) &serv_addr[i],
          &addr_len);
      if (n <= 0) {
